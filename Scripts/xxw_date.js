        ...
        bindEvent: function(type) {
            ...
            function popupDate(e) {
                ...
                _self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">' +
                    ...
                    '<p style="text-align: center;color: #ff6000;line-height: 2.888rem;font-size: 14px;" class="xxw_date_show"></p>'+
            ...
            //重置日期节点个数
            var xxw_dateTime_show = "";var xxw_date_show = "";var xxw_time_show = "";
            function setDateGearTooth() {
                ...
                if (date_yy && date_yy.getAttribute("val")) {
                    ...
                    xxw_date_show = xxw_date_show + parseInt(yyVal+_self.minY) +"/";
                    //p 当前节点前后需要展示的节点个数
                    for (var p = 0; p <= passY - 1; p++) {
                        if(yyVal==p){
                            itemStr += "<div class='tooth' style='color: #ff6000;height: 2.75rem;line-height: 2.75rem;'>" + (_self.minY +p) + "</div>";
                        }else{
                    ...
                    if (!isNaN(top)) {
                        ...
                        date_yy.style["-webkit-transform"] = 'translate3d(0,' + top + 'rem,0)';
                        date_yy.setAttribute('top', top + 'rem');
                        //yyVal = Math.abs(top - 8) / 2;
                        date_yy.setAttribute("val", yyVal);
                    } else {
                        date_yy.style["-webkit-transform"] = 'translate3d(0,' + (0 - (yyVal-2) * 1.757) + 'rem,0)';
                        date_yy.setAttribute('top', 0 - (yyVal-2) * 1.757 + 'rem');
                    }
                } else {
                    return;
                }
                ...
                if (date_mm && date_mm.getAttribute("val")) {
                    ...
                    date_mm.style["-webkit-transform"] = 'translate3d(0,' + (0 - (mmVal - minM-2) * 1.757) + 'rem,0)';
                    date_mm.setAttribute('top', 0 - (mmVal - minM-2) * 1.575 + 'rem');
                } else {
                    return;
                }
                var date_dd = _self.gearDate.querySelector(".date_dd");
                if (date_dd && date_dd.getAttribute("val")) {
                    ...
                    date_dd.style["-webkit-transform"] = 'translate3d(0,' + (0 - (ddVal - minD-2) * 1.757) + 'rem,0)';
                    date_dd.setAttribute('top', 0 - (ddVal - minD-2) * 1.757 + 'rem');
                } else {
                    return;
                }
                xxw_dateTime_show = xxw_date_show + xxw_time_show;
                $(".xxw_date_show").text(xxw_dateTime_show)
            }
            //重置时间节点个数
            function setTimeGearTooth() {
                ...
                if (time_hh && time_hh.getAttribute("val")) {
                    ...
                    time_hh.innerHTML = i;
                    time_hh.style["-webkit-transform"] = 'translate3d(0,' + (0 - (hhVal-2) * 1.757) + 'rem,0)';
                    time_hh.setAttribute('top', 0 - (hhVal-2) * 1.757 + 'rem');
                } else {
                    return
                }
                var time_mm = _self.gearDate.querySelector(".time_mm");
                if (time_mm && time_mm.getAttribute("val")) {
                    ...
                    time_mm.innerHTML = i;
                    time_mm.style["-webkit-transform"] = 'translate3d(0,' + (0 - (mmVal-2) * 1.757) + 'rem,0)';
                    time_mm.setAttribute('top', 0 - (mmVal-2) * 1.757 + 'rem');
                } else {
                    return
                }
                xxw_dateTime_show = xxw_date_show + xxw_time_show;
            ...
            function gearTouchMove(e) {
                ...
                var f =(target["new_" + target.id] - target["old_" + target.id])/parseInt($("body").css("fontSize").replace("px",""));
